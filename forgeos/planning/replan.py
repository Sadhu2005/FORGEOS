...
class Replanner:
    ...
    def on_failure(self, graph: TaskGraph, task: Task, error: str) -> ReplanResult:
        task.status = "FAILED"
        ...
        if task.attempts >= self.max_attempts:
            ...
            return ReplanResult(
                blocked=True,
                fix_task=None,
                message=f"blocked after {task.attempts} attempts: {error}",
            )
        ...
        fix = Task(
            id=fix_id,
            description=f"Fix after failure of {task.id}: {error[:120]}",
            status="READY",
        ...
        graph.add(fix)
        return ReplanResult(blocked=False, fix_task=fix, message=f"replan: added {fix_id}")
