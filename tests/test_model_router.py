...
from forgeos.llm.model_router import ModelRouter, RoutedLLM
...
def test_select_defaults() -> None:
    ...
    assert router.select("planning") == "qwen3:4b"


def test_planning_think_false() -> None:
    router = ModelRouter(OllamaClient())
    assert router.options_for("planning") == {"think": False}
